<script lang="ts">
interface States {
    model: TopPurchasedCustomerListModel;
    isInitialLoading: boolean;
}
</script>

<script setup lang="ts">
import { reactive, computed, onMounted } from "vue";
import { useStatsService } from "@/services/statsService";
import { TopPurchasedCustomerListModel } from "@/models/statsModels";
import { useInitialDataStore } from "@/stores/initialData";
import { useAmountUtility } from "@/utilities/amountUtility";

// Layout components.
import MainBlock from "../layouts/MainBlockComponent.vue";

// Dependency.
const statsService = useStatsService();
const initialDataStore = useInitialDataStore();
const amountUtility = useAmountUtility();

// States.
const states = reactive<States>({
    model: null!,
    isInitialLoading: true
});

// Computed properties.
const isCriteriaByPurchasedAmount = computed<boolean>(() => {
    return states.model.criteria === "PurchasedAmount";
});

// Life cycle hooks.
onMounted(async () => {
    await loadAsync();
    states.isInitialLoading = false
});

// Function.
async function loadAsync(): Promise<void> {
    const responseDto = await statsService.getTopPurchasedCustomerListAsync();
    states.model = new TopPurchasedCustomerListModel(
        responseDto,
        initialDataStore.data.stats.topPurchasedCustomer);
}

function getPurchasedAmountText(amount: number): string {
    return amountUtility.getDisplayText(amount);
}
</script>

<template>
    <MainBlock title="Khách hàng mua nhiều nhất" body-padding="0" class="h-100"
            body-class="h-100 overflow-hidden">
        <template v-if="!states.isInitialLoading">
            <ul class="list-group list-group-flush">
                <template v-if="states.model.items.length">
                    <li class="list-group-item d-flex align-items-center bg-transparent"
                            :key="index" v-for="(customer, index) in states.model.items">
                        <RouterLink :to="customer.detailRoute"
                                class="d-flex justify-content-center align-items-center
                                        flex-shrink-0 me-2">
                            <img class="img-thumbnail rounded-circle"
                                    :src="customer.avatarUrl">
                        </RouterLink>
                        <div class="flex-fill detail-container">
                            <RouterLink :to="customer.detailRoute" class="fw-bold">
                                {{ customer.fullName }}
                            </RouterLink><br/>
                            <span class="opacity-50 small">
                                <template v-if="isCriteriaByPurchasedAmount">
                                    {{ getPurchasedAmountText(customer.purchasedAmount) }}
                                </template>
                                <template v-else>
                                    {{ customer.purchasedTransactionCount }} giao dịch
                                </template>
                            </span>
                        </div>
                    </li>
                </template>
                <li class="list-group-item d-flex align-items-center justify-content-center
                            bg-transparent py-4 opacity-50"
                        v-else>
                    Không có sản phẩm nào
                </li>
            </ul>
        </template>
        <div class="d-flex justify-content-center align-items-center p-3" v-else>
            <div class="spinner-border text-primary" role="status">
                <span class="visually-hidden">Đang tải...</span>
            </div>
            <span class="ms-2 text-primary">Đang tải</span>
        </div>
    </MainBlock>
</template>

<style scoped>
.list-group-item {
    height: 65px;
}

.img-thumbnail {
    aspect-ratio: 1;
    height: 40px;
}

.detail-container {
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
}
</style>