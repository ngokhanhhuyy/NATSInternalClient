...
import { UpcomingBirthdayUserListModel } from "@/models";
...
// Functions
async function initializeModelAsync(): Promise<UpcomingBirthdayUserListModel> {
    const responseDto = await userService.getUpcomingBirthdayUsersAsync();
    return reactive(new UpcomingBirthdayUserListModel(responseDto));
}
...
<template>
    <MainBlock title="Sinh nhật tháng này" :body-padding="false">
        <!-- Header -->
        <template #header>
                ...
                <div v-if="!loadingState.isLoading">
                    <ul class="list-group list-group-flush top-effective-users-list"
                        ...
                        <li class="list-group-item d-flex flex-row align-items-center
                            ...
                            <span class="badge bg-success-subtle border border-success-subtle
                                        text-success rounded small ms-2 handled-orders fw-bold">
                                {{ user.daysLeftToBirthday }}
                            </span>
                        </li>
                    </ul>
                    <div class="d-flex align-items-center justify-content-center p-3
                                opacity-50"
                            v-else>
                        Không có nhân viên có sinh nhật sắp tới
                    </div>
                </div>
            </Transition>
        </template>
